...
pub async fn handle_work_conn(
    ...
    // 连接本地服务
    let local = tokio::net::TcpStream::connect(&local_addr)
        .await
        .map_err(|e| anyhow::anyhow!("connect {local_addr}: {e}"))?;

    // 将 QUIC stream 合并为单个双向 IO
    let quic_io = tokio::io::join(recv, send);

    metrics.conn_start();
    // 只在"本地服务 socket"一侧包计数器，避免在 quic_io 侧重复计数
    let mut local = CountingIo::wrap_local_side(local, metrics.clone());
    let mut quic_io = quic_io;
    let result = tokio::io::copy_bidirectional(&mut local, &mut quic_io).await;
    metrics.conn_end();
    result?;
...
pub async fn handle_work_conn_tcp(
    ...
    metrics.conn_start();
    let mut local = CountingIo::wrap_local_side(local, metrics.clone());
    let result = tokio::io::copy_bidirectional(&mut local, &mut work_io).await;
    metrics.conn_end();
    result?;
