...
pub struct AppState {
    ...
    /// 流量统计注册表，供 Web UI 展示；key 见 `metrics_key`
    pub metrics: MetricsRegistry,
}

/// server::run() 的返回值：AppState 供状态查询，listener_handles 供 hot-restart 时
/// 统一 abort（每个 listener 的 accept 循环各自是一个独立 tokio task）
...
impl ServerHandle {
    /// 停掉所有 listener task（用于热重启）
    pub fn abort_all(&self) {
        for h in &self.listener_handles {
            h.abort();
        }
    }
}

/// metrics 注册表的 key：同名 proxy 在不同 session 下要区分开
pub fn metrics_key(session_id: &str, proxy_name: &str) -> String {
    format!("{session_id}::{proxy_name}")
}

// ─── 入口：根据 transport 模式启动 listener ──────────────────────────────────

/// 启动所有 listener 并返回 `(AppState, 后台任务句柄)`。
/// `AppState` 立即可用（供 Web UI 查询 sessions/metrics），
/// 后台任务句柄在所有 listener 退出（或被 abort）时结束。
pub async fn run(
    cfg: ServerConfig,
    ...
    // 后台 task：定期清理已不存在 session 的残留 metrics（session 已从 sessions map
    // 移除，但 metrics 需要保留最后一次快照供 UI 短暂展示，故延迟到下一轮才清）
    {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let snap = metrics.snapshot_all().await;
                for key in snap.keys() {
                    if let Some((sid, _)) = key.split_once("::") {
                        if !live_ids.contains(sid) {
                            metrics.remove(key).await;
                        }
                    }
                }
            }
    ...
    // 先在 run() 里同步完成 bind（快速失败，热重启时能立刻拿到端口冲突等错误），
    // 再各自 spawn 一个后台 task 跑 accept 循环，并收集 JoinHandle 供热重启时统一 abort。
    let mut listener_handles = Vec::new();

    if transport.has_quic() {
        let endpoint = Endpoint::server(state.server_tls.quinn_cfg.clone(), addr)
            .map_err(|e| anyhow::anyhow!("QUIC bind {addr} failed: {e}"))?;
        info!("QUIC listening on {addr}");
        let s = state.clone();
        listener_handles.push(tokio::spawn(async move {
            if let Err(e) = run_quic(endpoint, s).await {
                warn!("QUIC listener ended: {e}");
            }
        }));
    }
    if transport.has_tcp() {
        let listener = TcpListener::bind(addr)
            .await
            .map_err(|e| anyhow::anyhow!("TCP bind {addr} failed: {e}"))?;
        info!("TCP+TLS listening on {addr}");
        let s = state.clone();
        listener_handles.push(tokio::spawn(async move {
            if let Err(e) = run_tcp(listener, s).await {
                warn!("TCP listener ended: {e}");
            }
        }));
    }
    if transport.has_websocket() {
        let listener = TcpListener::bind(addr)
            .await
            .map_err(|e| anyhow::anyhow!("WebSocket bind {addr} failed: {e}"))?;
        let s = state.clone();
        listener_handles.push(tokio::spawn(async move {
            if let Err(e) = crate::server::websocket::run_websocket(listener, s).await {
                warn!("WebSocket listener ended: {e}");
            }
        }));
    }
    if transport.has_xhttp() {
        let listener = TcpListener::bind(addr)
            .await
            .map_err(|e| anyhow::anyhow!("XHTTP bind {addr} failed: {e}"))?;
        let s = state.clone();
        listener_handles.push(tokio::spawn(async move {
            if let Err(e) = crate::server::xhttp::run_xhttp(listener, s).await {
                warn!("XHTTP listener ended: {e}");
            }
        }));
    }
...
async fn run_quic(endpoint: Endpoint, state: AppState) -> Result<()> {
...
pub async fn run_tcp(listener: TcpListener, state: AppState) -> Result<()> {
    let acceptor = state
        .tcp_tls_acceptor
        .clone()
            ...
            let result = if pc.r#type == ProxyType::Tcp as i32 {
                match pc.tcp {
                    ...
                    Some(tc) => {
                        ...
                        let metrics = self
                            .state
                            .metrics
                            .get_or_create(&metrics_key(&r.session_id, &pc.name))
                            .await;
                ...
                match pc.udp {
                    ...
                    Some(uc) => {
                        ...
                        tokio::spawn(async move {
                            udp::start_udp_proxy(name, local, port, sess, shutdown_rx, metrics)
                                .await;
                        });
