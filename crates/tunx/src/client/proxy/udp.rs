...
pub async fn handle_work_conn(
    ...
    let result: anyhow::Result<()> = loop {
        tokio::select! {
            // 本地 UDP → QUIC stream（加帧头）：这是客户端视角的“下行”回包
            // （数据从本地服务流向公网访问者）
            result = udp.recv(&mut buf) => {
                let n = match result { Ok(n) => n, Err(e) => break Err(e.into()) };
                let len = n as u16;
                let mut frame = Vec::with_capacity(2 + n);
                frame.extend_from_slice(&len.to_le_bytes());
                frame.extend_from_slice(&buf[..n]);
                if let Err(e) = send.write_all(&frame).await { break Err(e.into()); }
                metrics.add_down(n as u64);
            }
            // QUIC stream → 本地 UDP（解帧头）：客户端视角的“上行”请求
            result = recv.read_exact(&mut len_buf) => {
                if let Err(e) = result { break Err(e.into()); }
                let n = u16::from_le_bytes(len_buf) as usize;
                if n > MAX_UDP_PAYLOAD {
                    break Err(anyhow::anyhow!("UDP frame too large: {n}"));
                }
                if let Err(e) = recv.read_exact(&mut buf[..n]).await { break Err(e.into()); }
                if let Err(e) = udp.send(&buf[..n]).await { break Err(e.into()); }
    ...
    let result: anyhow::Result<()> = loop {
        tokio::select! {
            ...
            result = udp.recv(&mut buf) => {
                ...
                frame.extend_from_slice(&len.to_le_bytes());
                frame.extend_from_slice(&buf[..n]);
                if let Err(e) = work_io.write_all(&frame).await { break Err(e.into()); }
                metrics.add_down(n as u64);
            }
            // WorkConn → 本地 UDP（解帧头）：上行请求
            result = work_io.read_exact(&mut len_buf) => {
                ...
                if let Err(e) = work_io.read_exact(&mut buf[..n]).await { break Err(e.into()); }
                if let Err(e) = udp.send(&buf[..n]).await { break Err(e.into()); }
                metrics.add_up(n as u64);
            }
        }
