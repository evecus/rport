...
async fn handle_incoming_datagram(
    ...
    tokio::spawn(async move {
        if let Err(e) =
            run_udp_tunnel(name.clone(), peer, work_io, rx, socket, sessions.clone(), m).await
        {
...
async fn run_udp_tunnel(
    ...
    loop {
        tokio::select! {
            // 公网 → WorkConn（加帧头）
            // 注：上行字节数已在主循环 handle_incoming_datagram 调用处统一计数
            // （每个到达的数据报都会经过那里，无论是否新建会话），此处不再重复计数
