...
pub async fn run(cfg: ClientConfig, metrics: MetricsRegistry) -> Result<()> {
    ...
    loop {
        match run_session(&cfg, &client_id, &metrics).await {
...
async fn run_session(cfg: &ClientConfig, client_id: &str, metrics: &MetricsRegistry) -> Result<()> {
    match cfg.transport {
        ClientTransport::Quic => run_session_quic(cfg, client_id, metrics).await,
        ClientTransport::Tcp => run_session_tcp(cfg, client_id, metrics).await,
        ClientTransport::Websocket => run_session_websocket(cfg, client_id, metrics).await,
        ClientTransport::Xhttp => run_session_tcp(cfg, client_id, metrics).await, // XHTTP 复用 TCP 路径
    }
}

// ─── QUIC 路径 ───────────────────────────────────────────────────────────────

async fn run_session_quic(cfg: &ClientConfig, client_id: &str, metrics: &MetricsRegistry) -> Result<()> {
    // 1. 建立 QUIC 连接
        ...
        match stream.message().await {
            Ok(Some(msg)) => match msg.payload {
                ...
                Some(server_message::Payload::WorkConnReq(req)) => {
                    ...
                    let c = conn.clone();
                    let m = metrics.get_or_create(&proxy_name).await;
                    tokio::spawn(async move {
                        let result = if is_udp {
                            udp::handle_work_conn(c, proxy_name.clone(), local, stream_id, m)
                                .await
                        } else {
                            tcp::handle_work_conn(c, proxy_name.clone(), local, stream_id, m)
                                .await
                        };
                        if let Err(e) = result {
...
async fn run_session_tcp(cfg: &ClientConfig, client_id: &str, metrics: &MetricsRegistry) -> Result<()> {
    ...
    run_session_tcp_inner(channel, cfg, client_id, hostname, metrics).await
}
...
async fn run_session_websocket(cfg: &ClientConfig, client_id: &str, metrics: &MetricsRegistry) -> Result<()> {
    let hostname = cfg.tls_sni.clone().unwrap_or_else(|| {
        cfg.server_addr
            .split(':')
            .next()
